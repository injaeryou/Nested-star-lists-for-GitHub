...
named `parent/child` as a folder tree: on the profile's stars tab, as a rail
beside a single list's page, and inside both of the star button's list panels.

## Invariants

Break one of these and the failure is silent or global, so they come first.
...
- **Color goes through `--nsl-*`, never a raw value.** The tokens are defined
  once at the top of `styles.css` — Primer's own token first, a per-scheme
  fallback behind it — so the UI follows GitHub's explicit light/dark setting,
  which no media query can see. A fill baked into an SVG data URI follows
  nothing: glyphs are masks, painted with a token.

## The four surfaces

Two pages get a real `<details>` tree. Two panels get the same tree drawn inside
the rows GitHub rendered, because those rows are not equally ours to move.

- **The stars tab.** The lists index is only `/<user>?tab=stars` —
  `/stars/<user>/lists` is not a page — and it lives in
  `#profile-lists-container .Box`.
- **A single list's page.** `#user-list-repositories` is the only stable
  landmark: the first `h1` inside `main` is the profile name, and "looks like a
  repo link" heuristics hit a hidden report-abuse dialog.
- **A repo page's star panel — React.** Primer's `SelectPanel`: a
  `ul[role=listbox]` of `li[role=option]` with roving `aria-activedescendant`,
  re-rendered whole on every keystroke in its own filter. **Never reparent a
  row** — the next render calls `insertBefore` against a node that is no longer
  its child, and the panel dies. Inserting rows of our own is safe; React
  removes only what it created, and a closed folder hides its rows with
  `display:none` rather than moving them. Key the name memo on
  `data-id="list-<id>"`, never on the row: a re-render reuses one `<li>` for a
  different list. The hashed `prc-*` classes change every build.
- **Every other star panel — Catalyst.** Trending, the feed, the stars tab: a
  `<select-panel>` dialog whose rows live in an `<action-list>` —
  `li.ActionListItem` > `button.ActionListContent` > `.ActionListItem-label`,
  fetched only when the dialog is first opened. These rows *are* ours
  to move, and must be: the server's order is neither the tree's nor
  alphabetical. (The React panel already arrives sorted by full name, `/`
  sorting ahead of every letter, so `a/b` lands under `a` on its own.)
- **Both panels, in `picker.js` from one `PANELS` table.** A row's content is a
  CSS grid, so anything appended to it auto-places into a cell of its own — the
  caret under the checkbox, the count on a line by itself. The caret, the indent
  slot and the count hang off the `<li>` instead, laid out as a flex line. One
  page can hold many panels (trending has two per repo), so the pass works one
  `<ul>` at a time, never across the page.
...
column to a stacked block at 1011px, and some assertions branch on that. Check
it in both color schemes too — the token fallbacks differ, and only a page
without Primer's tokens (this one) exercises them.
...
```js
for (const f of ['src/core.js', 'src/settings.js', 'src/rail.js', 'src/picker.js',
                 'src/boot.js'])
...
- **Fixture fidelity.** A fixture that is simpler than the real markup proves
  nothing about it: a panel row laid out as a flex line hid a whole class of
  grid bugs, and a description rendered inline hid a two-line row.
...
- Comments in English, and only where the code cannot say it — mostly *why*, not
  *what*.
- Reuse GitHub's own classes (`Box`, `Counter`, `ActionList*`) so the UI is
  themed and spaced like the page around it.
...
- **The star-list API exists but is not worth its price here.** GraphQL has
  ...
  discussion #8293); what it costs is the zero-storage promise (the token has to
  live somewhere) and works-on-install onboarding (create, scope, paste,
  expire). Far more capability than the read path needs, for a marginal gain.
  Stay on the same-origin `fetch` of the lists index. The mutations are the only
  thing worth wanting (renaming a list into `parent/child` from the UI) — and
  that is opt-in, not a rewrite of how lists are read.
